# Phase 4e — Preloading: pod.Model → []playerPodRole.Model
...
- `lib/business/pod/functions.go` — optional: `GetByID` variant that returns members

Depends on: Phase 2c (pod), Phase 2d (playerPodRole) GORM-migrated
...
Optionally, if a combined pod+members fetch is ever needed, a new business function
can call `GetByIDWithMembers` and convert the preloaded `Members` slice:
...
Adding this function and the `EntityWithMembers` type, `GetByIDWithMembersFunc`, and
`Functions` field update is optional for this phase — only add if there is a concrete
caller at implementation time.
