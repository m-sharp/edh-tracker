# Phase 1 — Preloading: game.Model → []gameResult.Model
...
that is addressed in Phase 3.

## Dependencies

None — standalone, can run before or in parallel with Phase 2.
...
- `lib/business/gameResult/types.go` — add `EnrichModelsFunc` type + field to `Functions`
...
until Phase 3 replaces them with preloaded deck data.
...
Per-result enrichment (deck name, commander, player ID) unchanged — addressed in Phase 3.
